    ...
    if i == 0:
        for j in range(COLS):
            if j == 0:
                ...
                CARD_GRID[i].append(
                    pygame.Rect(CARD_GRID[i][j - 1].x + CARD_LEN + CARD_MARGIN, CARD_MARGIN, CARD_LEN, CARD_LEN))
    else:
        for j in range(COLS):
            if j == 0:
                CARD_GRID[i].append(
                    pygame.Rect(CARD_MARGIN, CARD_GRID[i - 1][0].y + CARD_LEN + CARD_MARGIN, CARD_LEN, CARD_LEN))
            else:
                CARD_GRID[i].append(pygame.Rect(CARD_GRID[i][j - 1].x + CARD_LEN + CARD_MARGIN,
                                                CARD_GRID[i - 1][0].y + CARD_LEN + CARD_MARGIN, CARD_LEN, CARD_LEN))
global exposed
exposed = []
global matched
matched = []
        ...
        if pressed[i]:
            for i in range(ROWS):
                for j in range(COLS):
                    ...
                    if mouse_pos[0] >= CARD_GRID[i][j].x and mouse_pos[1] >= CARD_GRID[i][j].y and mouse_pos[0] <= \
                            CARD_GRID[i][j].x + CARD_LEN and mouse_pos[1] <= CARD_GRID[i][j].y + CARD_LEN:
                        global has_instance
                        has_instance = False
